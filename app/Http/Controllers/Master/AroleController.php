...
class AroleController extends Controller
{
    /**
    ...
    public function store(Request $request)
    {
        ...
        $k =  ARoleModel::updateOrCreate([
            'id' => $request->roleid
        ], [
            'nama' => $request->rolenama,
            'urutan' => $request->roleurutan,
        ]);
        if ($k) {
    ...
    public function destroy(string $id)
    {
        session()->put('tab', 5);
        session()->put('subtab', 0);
        $k = ARoleModel::where('id', $id)->delete();
        if ($k) {
